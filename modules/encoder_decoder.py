...
class Encoder(nn.Module):
    def __init__(self, layer, N):
        ...
        self.MLP = nn.Sequential(
            nn.Linear(N * layer.d_model, N * layer.d_model),
            nn.LeakyReLU(),
            nn.Linear(N * layer.d_model, layer.d_model),
            nn.LeakyReLU()
        )
...
class DecoderLayer(nn.Module):
    def __init__(self,
        ...
        self.memory_module = MemoryMatrixModule(
            rm_num_slots, rm_d_model, d_model, dropout=dropout)
        self.feed_forward = feed_forward
        self.sublayer = clones(SublayerConnection(d_model, dropout), 3)
        self.gate_proj = nn.Linear(d_model, 1)
        self.gamma     = nn.Parameter(torch.tensor(0.4))

    def forward(self, x, hidden_states, src_mask=None, tgt_mask=None):
        m = hidden_states
        x = self.sublayer[0](x, lambda t: self.self_attn(t, t, t, tgt_mask))
        x = self.sublayer[1](x, lambda t: self.src_attn(t, m, m, src_mask))
        mem_out = self.memory_module(x)
        gate    = torch.sigmoid(self.gate_proj(x))
        x       = x + 0.5 * self.gamma * gate * mem_out
        return self.sublayer[2](x, self.feed_forward)


class MemoryMatrixModule(nn.Module):
    def __init__(self,
                 rm_num_slots: int,
                 rm_d_model:   int,
                 d_model:      int,
        ...
        self.memory = nn.Parameter(torch.randn(rm_num_slots, rm_d_model))
        self.q_proj   = nn.Linear(d_model,      d_model,      bias=False)
        self.k_proj   = nn.Linear(rm_d_model,   d_model,      bias=False)
        self.v_proj   = nn.Linear(rm_d_model,   d_model,      bias=False)
        self.out_proj = nn.Linear(d_model,      d_model,      bias=False)
        self.norm = nn.LayerNorm(rm_d_model)
        self.dropout = nn.Dropout(dropout)
        self.drop_path = DropPath(dropout) if dropout > 0.0 else nn.Identity()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, L, _ = x.shape
        mem = self.dropout(self.norm(self.memory))
        q = self.q_proj(x).view(B, L, self.h, self.d_head).transpose(1, 2)
        k = self.k_proj(mem).view(self.rm_num_slots, self.h, self.d_head).permute(1, 0, 2)
        v = self.v_proj(mem).view(self.rm_num_slots, self.h, self.d_head).permute(1, 0, 2)
